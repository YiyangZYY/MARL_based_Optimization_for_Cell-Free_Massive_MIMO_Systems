...
class MPNN_GNN3D_L_K_U_Power(nn.Module):
    def __init__(self, input_dim_1, input_dim_2, hidden_dim, output_dim, device, BATCH_SIZE):
        super(MPNN_GNN3D_L_K_U_Power, self).__init__()
        self.out = nn.Sigmoid()
        self.layers = torch.nn.ModuleList()
        self.BATCH_SIZE = BATCH_SIZE
        self.device = device
    ...
    def forward(self, Hhat, nbrOfRealizations, L, K, U):
        ...
        # to satisfy constraints
        Fhat_u1 = V_AP_output[:, :2].view([BATCH_SIZE * nbrOfRealizations, 2, L, K, U])[:, 0]
        Fhat_u2 = V_AP_output[:, :2].view([BATCH_SIZE * nbrOfRealizations, 2, L, K, U])[:, 1]
        Fhat_u3 = torch.tile(torch.sqrt(torch.sum(Fhat_u1 ** 2, dim=3) + torch.sum(Fhat_u2 ** 2, dim=3)).view(BATCH_SIZE * nbrOfRealizations, L, K, 1), (1, 1, 1, U))
        Fhat_u = (Fhat_u1 + 1j * Fhat_u2) / Fhat_u3
        Phat_u1 = V_AP_output[:, 2:].view([BATCH_SIZE, nbrOfRealizations, L, K, U])
...
class MPNN_GNN2D_L_K_Power(nn.Module):
    def __init__(self, input_dim_1, input_dim_2, hidden_dim, output_dim, device, BATCH_SIZE):
        super(MPNN_GNN2D_L_K_Power, self).__init__()
        self.out = nn.Sigmoid()
        self.layers = torch.nn.ModuleList()
        self.BATCH_SIZE = BATCH_SIZE
        self.device = device
    ...
    def forward(self, Hhat, nbrOfRealizations, L, K, U):
        ...
        # to satisfy constraints
        Fhat_u1 = V_AP_output[:, :2 * U].view([BATCH_SIZE * nbrOfRealizations, 2, U, L, K])[:, 0].transpose(1, 2).transpose(2, 3)
        Fhat_u2 = V_AP_output[:, :2 * U].view([BATCH_SIZE * nbrOfRealizations, 2, U, L, K])[:, 1].transpose(1, 2).transpose(2, 3)
        Fhat_u3 = torch.tile(torch.sqrt(torch.sum(Fhat_u1 ** 2, dim=3) + torch.sum(Fhat_u2 ** 2, dim=3)).view(BATCH_SIZE * nbrOfRealizations, L, K, 1), (1, 1, 1, U))
        Fhat_u = (Fhat_u1 + 1j * Fhat_u2) / Fhat_u3
        Phat_u1 = V_AP_output[:, 2 * U:].view([BATCH_SIZE, nbrOfRealizations, L, K])
        Phat_u = Phat_u1 / torch.linalg.norm(Phat_u1, dim=2).view([BATCH_SIZE, nbrOfRealizations, 1, K]) * math.sqrt(L)
        Fhat_u = Fhat_u.view([BATCH_SIZE, nbrOfRealizations, L, K, U]) * Phat_u.view([BATCH_SIZE, nbrOfRealizations, L, K, 1])
        return Fhat_u
...
class MPNN_GNN1D_L_Power(nn.Module):
    def __init__(self, input_dim_1, input_dim_2, hidden_dim, output_dim, device, BATCH_SIZE):
        super(MPNN_GNN1D_L_Power, self).__init__()
        self.out = nn.Sigmoid()
        self.layers = torch.nn.ModuleList()
        self.BATCH_SIZE = BATCH_SIZE
        self.device = device
    ...
    def forward(self, Hhat, nbrOfRealizations, L, K, U):
        ...
        # to satisfy constraints
        Fhat_u1 = V_AP_output[:, :2 * K * U].view([BATCH_SIZE * nbrOfRealizations, 2, K, U, L])[:, 0].transpose(1, 3).transpose(2, 3)
        Fhat_u2 = V_AP_output[:, :2 * K * U].view([BATCH_SIZE * nbrOfRealizations, 2, K, U, L])[:, 1].transpose(1, 3).transpose(2, 3)
        Fhat_u3 = torch.tile(torch.sqrt(torch.sum(Fhat_u1 ** 2, dim=3) + torch.sum(Fhat_u2 ** 2, dim=3)).view(BATCH_SIZE * nbrOfRealizations, L, K, 1), (1, 1, 1, U))
        Fhat_u = (Fhat_u1 + 1j * Fhat_u2) / Fhat_u3
        Phat_u1 = V_AP_output[:, 2 * K * U:].view([BATCH_SIZE, nbrOfRealizations, K, L]).transpose(2, 3)
        Phat_u = Phat_u1 / torch.linalg.norm(Phat_u1, dim=2).view([BATCH_SIZE, nbrOfRealizations, 1, K]) * math.sqrt(L)
        Fhat_u = Fhat_u.view([BATCH_SIZE, nbrOfRealizations, L, K, U]) * Phat_u.view([BATCH_SIZE, nbrOfRealizations, L, K, 1])
        return Fhat_u


class MPNN_GNN1D_K_Power(nn.Module):
    def __init__(self, input_dim_1, input_dim_2, hidden_dim, output_dim, device, BATCH_SIZE):
        super(MPNN_GNN1D_K_Power, self).__init__()
        self.out = nn.Sigmoid()
        self.layers = torch.nn.ModuleList()
        self.BATCH_SIZE = BATCH_SIZE
        self.device = device
    ...
    def forward(self, Hhat, nbrOfRealizations, L, K, U):
        ...
        # to satisfy constraints
        Fhat_u1 = V_AP_output[:, :2 * L * U].view([BATCH_SIZE * nbrOfRealizations, 2, L, U, K])[:, 0].transpose(2, 3)
        Fhat_u2 = V_AP_output[:, :2 * L * U].view([BATCH_SIZE * nbrOfRealizations, 2, L, U, K])[:, 1].transpose(2, 3)
        Fhat_u3 = torch.tile(torch.sqrt(torch.sum(Fhat_u1 ** 2, dim=3) + torch.sum(Fhat_u2 ** 2, dim=3)).view(BATCH_SIZE * nbrOfRealizations, L, K, 1), (1, 1, 1, U))
        Fhat_u = (Fhat_u1 + 1j * Fhat_u2) / Fhat_u3
        Phat_u1 = V_AP_output[:, 2 * L * U:].view([BATCH_SIZE, nbrOfRealizations, L, K])
        Phat_u = Phat_u1 / torch.linalg.norm(Phat_u1, dim=2).view([BATCH_SIZE, nbrOfRealizations, 1, K]) * math.sqrt(L)
        Fhat_u = Fhat_u.view([BATCH_SIZE, nbrOfRealizations, L, K, U]) * Phat_u.view([BATCH_SIZE, nbrOfRealizations, L, K, 1])
        return Fhat_u


class MPNN_GNN1D_U_Power(nn.Module):
    def __init__(self, input_dim_1, input_dim_2, hidden_dim, output_dim, device, BATCH_SIZE):
        super(MPNN_GNN1D_U_Power, self).__init__()
        self.out = nn.Sigmoid()
        self.layers = torch.nn.ModuleList()
        self.BATCH_SIZE = BATCH_SIZE
        self.device = device
    ...
    def forward(self, Hhat, nbrOfRealizations, L, K, U):
        ...
        # to satisfy constraints
        Fhat_u1 = V_AP_output[:, :2 * L * K].view([BATCH_SIZE * nbrOfRealizations, 2, L, K, U])[:, 0]
        Fhat_u2 = V_AP_output[:, :2 * L * K].view([BATCH_SIZE * nbrOfRealizations, 2, L, K, U])[:, 1]
        Fhat_u3 = torch.tile(torch.sqrt(torch.sum(Fhat_u1 ** 2, dim=3) + torch.sum(Fhat_u2 ** 2, dim=3)).view(BATCH_SIZE * nbrOfRealizations, L, K, 1), (1, 1, 1, U))
        Fhat_u = (Fhat_u1 + 1j * Fhat_u2) / Fhat_u3
        Phat_u1 = V_AP_output[:, 2 * L * K:].view([BATCH_SIZE, nbrOfRealizations, L, K, U])
        Phat_u2 = torch.mean(Phat_u1, dim=4)
        Phat_u = Phat_u2 / torch.linalg.norm(Phat_u2, dim=2).view([BATCH_SIZE, nbrOfRealizations, 1, K]) * math.sqrt(L)
        Fhat_u = Fhat_u.view([BATCH_SIZE, nbrOfRealizations, L, K, U]) * Phat_u.view([BATCH_SIZE, nbrOfRealizations, L, K, 1])
        return Fhat_u
...
class GNNA3D_L_K_U_Power(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, device, BATCH_SIZE):
        super(GNNA3D_L_K_U_Power, self).__init__()
        self.out = nn.Sigmoid()
        self.layers = torch.nn.ModuleList()
        self.BATCH_SIZE = BATCH_SIZE
        self.device = device
...
class GNNA2D_L_K_Power(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, device, BATCH_SIZE):
        super(GNNA2D_L_K_Power, self).__init__()
        self.out = nn.Sigmoid()
        self.layers = torch.nn.ModuleList()
        self.BATCH_SIZE = BATCH_SIZE
        self.device = device
...
class Layer_1DPE_attention(nn.Module):
    ...
    def forward(self, A, permutation_size1, BATCH_SIZE, aggr_func=torch.mean):
        ...
        Alpha_norm = abs(Alpha) / torch.linalg.norm(Alpha, dim=3).view([BATCH_SIZE, self.output_dim, permutation_size1, 1])
        # print(torch.linalg.norm(Alpha, dim=3))
...
class GNNA1D_L_Power(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, device, BATCH_SIZE):
        super(GNNA1D_L_Power, self).__init__()
        self.out = nn.Sigmoid()
        self.layers = torch.nn.ModuleList()
        self.BATCH_SIZE = BATCH_SIZE
        self.device = device
...
class GNN1D_U_Power(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, device, BATCH_SIZE):
        super(GNN1D_U_Power, self).__init__()
        self.out = nn.Sigmoid()
        self.layers = torch.nn.ModuleList()
        self.BATCH_SIZE = BATCH_SIZE
        self.device = device
...
class GNN2D_K_U_Power(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, device, BATCH_SIZE):
        super(GNN2D_K_U_Power, self).__init__()
        self.out = nn.Sigmoid()
        self.layers = torch.nn.ModuleList()
        self.BATCH_SIZE = BATCH_SIZE
        self.device = device
...
class GNN2D_L_U_Power(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, device, BATCH_SIZE):
        super(GNN2D_L_U_Power, self).__init__()
        self.out = nn.Sigmoid()
        self.layers = torch.nn.ModuleList()
        self.BATCH_SIZE = BATCH_SIZE
        self.device = device
...
class GNN3D_L_K_U_Power(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, device, BATCH_SIZE):
        super(GNN3D_L_K_U_Power, self).__init__()
        self.out = nn.Sigmoid()
        self.layers = torch.nn.ModuleList()
        self.BATCH_SIZE = BATCH_SIZE
        self.device = device
