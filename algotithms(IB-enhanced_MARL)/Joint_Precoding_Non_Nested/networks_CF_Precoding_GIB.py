    ...
    def forward(self, Hhat, nbrOfRealizations, L, K, U):
        ...
        for i in range(len(self.dim)-1):
            if i != len(self.dim) - 2:
                ...
                V_AP_output, V_UE_output, _, _ = self.layers[i](V_AP_updated, V_UE_updated, Hhat_updated, permutation_size1=L, permutation_size2=K, BATCH_SIZE=BATCH_SIZE * nbrOfRealizations)

        # to satisfy constraints
        Fhat_u1 = V_AP_output[:, :2 * U].view([BATCH_SIZE * nbrOfRealizations, 2, 1, U, L])[:, 0].transpose(1, 3).transpose(2, 3)
        Fhat_u2 = V_AP_output[:, :2 * U].view([BATCH_SIZE * nbrOfRealizations, 2, 1, U, L])[:, 1].transpose(1, 3).transpose(2, 3)
        Fhat_u1 = V_UE_output[:, :2 * U].view([BATCH_SIZE * nbrOfRealizations, 2, 1, U, K])[:, 0].transpose(2, 3) + Fhat_u1
        Fhat_u2 = V_UE_output[:, :2 * U].view([BATCH_SIZE * nbrOfRealizations, 2, 1, U, K])[:, 1].transpose(2, 3) + Fhat_u2
        ...
        Phat_u1 = V_AP_output[:, 2 * U:].view([BATCH_SIZE, nbrOfRealizations, K, L]).transpose(2, 3)
...
class VIB_GNN(nn.Module):
    def __init__(self, input_dim_1, input_dim_2, hidden_dim, output_dim, sample_size, device, BATCH_SIZE):
        super(VIB_GNN, self).__init__()
        self.out = nn.Sigmoid()
        self.layers = torch.nn.ModuleList()
        self.BATCH_SIZE = BATCH_SIZE
        self.device = device
...
class VGIB_GNN_Power(nn.Module):
    ...
    def forward(self, Hhat, Ahat_AP_reshape, GFA_AP, Ahat_UE_reshape, GFA_UE, nbrOfRealizations, L, K, U):
        # consider the permutations of users, antennas
        BATCH_SIZE = self.BATCH_SIZE
        Hhat1 = Hhat.real.view(BATCH_SIZE * nbrOfRealizations, 1, L, K, U).transpose(2, 3).transpose(2, 4)
        Hhat2 = Hhat.imag.view(BATCH_SIZE * nbrOfRealizations, 1, L, K, U).transpose(2, 3).transpose(2, 4)
        ...
        for i in range(len(self.dim)-1):
            if i != len(self.dim) - 2:
                ...
                V_AP_output, V_UE_output, _, _, _, _ = \
                    self.layers[i](V_AP_updated, V_UE_updated, Hhat_updated, Ahat_AP_reshape, GFA_AP, Ahat_UE_reshape, GFA_UE, permutation_size1=L, permutation_size2=K, BATCH_SIZE=BATCH_SIZE * nbrOfRealizations)
...
class VGIB_GNN(nn.Module):
    def __init__(self, input_dim_1, input_dim_2, hidden_dim, output_dim, sample_size, device, BATCH_SIZE):
        super(VGIB_GNN, self).__init__()
        self.out = nn.Sigmoid()
        self.layers = torch.nn.ModuleList()
        self.BATCH_SIZE = BATCH_SIZE
        self.device = device
    ...
    def forward(self, Hhat, Ahat_AP_reshap, Ahat_UE_reshape, nbrOfRealizations, L, K, U):
        # consider the permutations of users, antennas
        BATCH_SIZE = self.BATCH_SIZE
        Hhat1 = Hhat.real.view(BATCH_SIZE * nbrOfRealizations, 1, L, K, U).transpose(2, 3).transpose(2, 4)
        Hhat2 = Hhat.imag.view(BATCH_SIZE * nbrOfRealizations, 1, L, K, U).transpose(2, 3).transpose(2, 4)
        ...
        I_XZ_AP = torch.zeros(len(self.dim) - 2, BATCH_SIZE * nbrOfRealizations).float().to(self.device)
        I_XZ_UE = torch.zeros(len(self.dim) - 2, BATCH_SIZE * nbrOfRealizations).float().to(self.device)
        I_AZ_AP = torch.zeros(len(self.dim) - 2).float().to(self.device)
        I_AZ_UE = torch.zeros(len(self.dim) - 2).float().to(self.device)

        for i in range(len(self.dim)-1):
            if i != len(self.dim) - 2:
                V_AP_updated, V_UE_updated, I_XZ_AP[i, :], I_XZ_UE[i, :], I_AZ_AP[i], I_AZ_UE[i] = \
                    self.layers[i](V_AP_updated, V_UE_updated, Hhat_updated, Ahat_AP_reshap, Ahat_UE_reshape, permutation_size1=L, permutation_size2=K, BATCH_SIZE=BATCH_SIZE * nbrOfRealizations)
            else:
                V_AP_output, V_UE_output, _, _, _, _ = \
                    self.layers[i](V_AP_updated, V_UE_updated, Hhat_updated, Ahat_AP_reshap, Ahat_UE_reshape, permutation_size1=L, permutation_size2=K, BATCH_SIZE=BATCH_SIZE * nbrOfRealizations)
...
class EIB_GNN1D_K_Power(nn.Module):
    ...
    def forward(self, Hhat, nbrOfRealizations, L, K, U):
        ...
        # to satisfy constraints
        Fhat_u1 = Hhat_updated[:, :2 * L * U].view([BATCH_SIZE * nbrOfRealizations, 2, L, U, K])[:, 0].transpose(2, 3)
        Fhat_u2 = Hhat_updated[:, :2 * L * U].view([BATCH_SIZE * nbrOfRealizations, 2, L, U, K])[:, 1].transpose(2, 3)
        Fhat_u3 = torch.tile(torch.sqrt(torch.sum(Fhat_u1 ** 2, dim=3) + torch.sum(Fhat_u2 ** 2, dim=3)).view(BATCH_SIZE * nbrOfRealizations, L, K, 1), (1, 1, 1, U))
        Fhat_u = (Fhat_u1 + 1j * Fhat_u2) / Fhat_u3
        Phat_u1 = Hhat_updated[:, 2 * L * U:].view([BATCH_SIZE, nbrOfRealizations, L, K])
        Phat_u = Phat_u1 / torch.linalg.norm(Phat_u1, dim=2).view([BATCH_SIZE, nbrOfRealizations, 1, K]) * math.sqrt(L)
        Fhat_u = Fhat_u.view([BATCH_SIZE, nbrOfRealizations, L, K, U]) * Phat_u.view([BATCH_SIZE, nbrOfRealizations, L, K, 1])
...
class EIB_GNN1D_U_Power(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, sample_size, device, BATCH_SIZE):
        super(EIB_GNN1D_U_Power, self).__init__()
        self.out = nn.Sigmoid()
        self.layers = torch.nn.ModuleList()
        self.sample_size = sample_size
        self.BATCH_SIZE = BATCH_SIZE
    ...
    def forward(self, Hhat, nbrOfRealizations, L, K, U):
        ...
        for i in range(len(self.dim)-1):
            if i != len(self.dim) - 2:
                Hhat_updated, I_XZ[i, :] = self.layers[i](Hhat_updated, permutation_size1=U, BATCH_SIZE=BATCH_SIZE * nbrOfRealizations)
            else:
                Hhat_updated, _ = self.layers[i](Hhat_updated, permutation_size1=U, BATCH_SIZE=BATCH_SIZE * nbrOfRealizations)

        # to satisfy constraints
        Fhat_u1 = Hhat_updated[:, :2 * L * K].view([BATCH_SIZE * nbrOfRealizations, 2, L, K, U])[:, 0]
        Fhat_u2 = Hhat_updated[:, :2 * L * K].view([BATCH_SIZE * nbrOfRealizations, 2, L, K, U])[:, 1]
        Fhat_u3 = torch.tile(torch.sqrt(torch.sum(Fhat_u1 ** 2, dim=3) + torch.sum(Fhat_u2 ** 2, dim=3)).view(BATCH_SIZE * nbrOfRealizations, L, K, 1), (1, 1, 1, U))
        Fhat_u = (Fhat_u1 + 1j * Fhat_u2) / Fhat_u3
        Phat_u1 = Hhat_updated[:, 2 * L * K:].view([BATCH_SIZE, nbrOfRealizations, L, K, U])
...
class EIB_GNN2D_K_U_Power(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, sample_size, device, BATCH_SIZE):
        super(EIB_GNN2D_K_U_Power, self).__init__()
        self.out = nn.Sigmoid()
        self.layers = torch.nn.ModuleList()
        self.sample_size = sample_size
        self.BATCH_SIZE = BATCH_SIZE
    ...
    def forward(self, Hhat, nbrOfRealizations, L, K, U):
        ...
        for i in range(len(self.dim)-1):
            if i != len(self.dim) - 2:
                Hhat_updated, I_XZ[i, :] = self.layers[i](Hhat_updated, permutation_size1=K, permutation_size2=U, BATCH_SIZE=BATCH_SIZE * nbrOfRealizations)
            else:
                Hhat_updated, _ = self.layers[i](Hhat_updated, permutation_size1=K, permutation_size2=U, BATCH_SIZE=BATCH_SIZE * nbrOfRealizations)

        # to satisfy constraints
        Fhat_u1 = Hhat_updated[:, :2 * L].view([BATCH_SIZE * nbrOfRealizations, 2, L, K, U])[:, 0]
        Fhat_u2 = Hhat_updated[:, :2 * L].view([BATCH_SIZE * nbrOfRealizations, 2, L, K, U])[:, 1]
        Fhat_u3 = torch.tile(torch.sqrt(torch.sum(Fhat_u1 ** 2, dim=3) + torch.sum(Fhat_u2 ** 2, dim=3)).view(BATCH_SIZE * nbrOfRealizations, L, K, 1), (1, 1, 1, U))
        Fhat_u = (Fhat_u1 + 1j * Fhat_u2) / Fhat_u3
        Phat_u1 = Hhat_updated[:, 2 * L:].view([BATCH_SIZE, nbrOfRealizations, L, K, U])
        Phat_u2 = torch.mean(Phat_u1, dim=4)
        Phat_u = Phat_u2 / torch.linalg.norm(Phat_u2, dim=2).view([BATCH_SIZE, nbrOfRealizations, 1, K]) * math.sqrt(L)
        Fhat_u = Fhat_u.view([BATCH_SIZE, nbrOfRealizations, L, K, U]) * Phat_u.view([BATCH_SIZE, nbrOfRealizations, L, K, 1])
        # to compute upper bound of I(X;Z)
...
class EIB_GNN2D_L_U_Power(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, sample_size, device, BATCH_SIZE):
        super(EIB_GNN2D_L_U_Power, self).__init__()
        self.out = nn.Sigmoid()
        self.layers = torch.nn.ModuleList()
        self.sample_size = sample_size
        self.BATCH_SIZE = BATCH_SIZE
...
class EIB_GNN2D_L_K(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, sample_size, device, BATCH_SIZE):
        super(EIB_GNN2D_L_K, self).__init__()
        self.out = nn.Sigmoid()
        self.layers = torch.nn.ModuleList()
        self.sample_size = sample_size
        self.BATCH_SIZE = BATCH_SIZE
...
class Layer_EIB_3DPE(nn.Module):
    def __init__(self, input_dim, output_dim, sample_size, device, transfer_function=nn.ReLU(), is_BN=True, is_transfer=True):
        super(Layer_EIB_3DPE, self).__init__()
    ...
    def forward(self, A, permutation_size1, permutation_size2, permutation_size3, BATCH_SIZE, aggr_func=torch.mean):
        # EGIB: input(Graph_Edge_reshape) + I_AZ -> VGIB
        # # lower bound of I(Y; Z) - edge_index, x_j_b -> x_i_a
        # edge_index_j, edge_index_b, edge_index_y = Graph_Edge_reshape[:, 0, 0].to(torch.int64), Graph_Edge_reshape[:, 0, 1].to(torch.int64),\
        #                                             Graph_Edge_reshape[:, 0, 2].to(torch.int64)
        # edge_index_i, edge_index_a, edge_index_x = Graph_Edge_reshape[:, 1, 0].to(torch.int64), Graph_Edge_reshape[:, 1, 1].to(torch.int64), \
        #                                             Graph_Edge_reshape[:, 1, 2].to(torch.int64)
        #
        # # Message - edge_index, x_j_b_y -> x_i_a_x: Batch_size * num_edge * self.input_dim_1
        # edge_x_i_a_x_tot = A[:, :, edge_index_i, edge_index_a, edge_index_x].transpose(1, 2).diagonal().transpose(0, 2)
        # edge_x_j_b_y_tot = A[:, :, edge_index_j, edge_index_b, edge_index_y].transpose(1, 2).diagonal().transpose(0, 2)
        # edge_x_j_b_y_tot, alpha, prior = neighbor_sampling(edge_x_i_a_x_tot, edge_x_j_b_y_tot, self.Att_Edge, BATCH_SIZE, self.device)
        #
        # A = torch.sum(edge_x_j_b_y_tot[:, GFA_Edge.to(torch.int64), :].diagonal(), dim=3).transpose(0, 4).transpose(1, 3).transpose(2, 4)
        #
        # # AIB - KL_Divergence - Categorical / Bernoulli
        # if 'categorical' in self.neighbor_sampling_mode:
        #     I_AZ = torch.sum(alpha * torch.log((alpha + 1e-16) / prior), dim=1)
        # else:
        #     I_AZ = kl_divergence(Bernoulli(alpha), Bernoulli(prior)).sum(-1)

        A1 = torch.matmul(self.P1, A.view([BATCH_SIZE, self.input_dim, -1])).view([BATCH_SIZE, self.output_dim * 2, permutation_size1, permutation_size2, permutation_size3])
        A2 = torch.matmul(self.P2, aggr_func(A, -1).view(BATCH_SIZE, self.input_dim, -1)).view(BATCH_SIZE, self.output_dim * 2, permutation_size1, permutation_size2, 1)
        A3 = torch.matmul(self.P3, aggr_func(A, -2).view(BATCH_SIZE, self.input_dim, -1)).view(BATCH_SIZE, self.output_dim * 2, permutation_size1, 1, permutation_size3)
        A4 = torch.matmul(self.P4, aggr_func(A, -3).view(BATCH_SIZE, self.input_dim, -1)).view(BATCH_SIZE, self.output_dim * 2, 1, permutation_size2, permutation_size3)
...
class EIB_GNN3D_L_K_U_Power(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, sample_size, device, BATCH_SIZE):
        super(EIB_GNN3D_L_K_U_Power, self).__init__()
        self.out = nn.Sigmoid()
        self.layers = torch.nn.ModuleList()
        self.sample_size = sample_size
        self.BATCH_SIZE = BATCH_SIZE
        ...
        for i in range(len(self.dim) - 1):
            if i != len(self.dim) - 2:
                self.layers.append(Layer_EIB_3DPE(self.dim[i], self.dim[i + 1], sample_size, device, transfer_function=nn.ReLU(), is_BN=True, is_transfer=True))
            else:
                self.layers.append(Layer_EIB_3DPE(self.dim[i], self.dim[i + 1], sample_size, device, transfer_function=nn.ReLU(), is_BN=False, is_transfer=False))
...
class EIB_GNN3D_L_K_U(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, sample_size, device, BATCH_SIZE):
        super(EIB_GNN3D_L_K_U, self).__init__()
        self.out = nn.Sigmoid()
        self.layers = torch.nn.ModuleList()
        self.sample_size = sample_size
        self.BATCH_SIZE = BATCH_SIZE
